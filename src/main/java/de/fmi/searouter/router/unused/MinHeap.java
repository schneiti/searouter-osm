...
public class MinHeap {
    ...
    private final HeapComparator heapCompare;

    /**
     * constructor for the dijkstra heap
     * @param heapCompare HeapComparator for comparing the distances of nodes in the current routing state
     */
    public MinHeap(HeapComparator heapCompare) {
        this.heapPosition = new int[Node.getSize()];
        this.idHeapArray = new int[INITIAL_SIZE];
        Arrays.fill(idHeapArray, -1);
        this.heapCompare = heapCompare;
        currentSize = 0;
    }
    ...
    private int compareValues(int firstID, int secondID) {
        return heapCompare.compareValues(idHeapArray[firstID], idHeapArray[secondID]);
    }
}
